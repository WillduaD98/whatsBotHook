...
// Modelo de la cola: cada mensaje entrante se guarda como WebhookEvent para procesarlo después
import { WebhookEvent } from "../models/WebhookEvent.js";
// Worker de la cola: dispara el procesamiento del waId recién encolado
import { scheduleProcessing } from "../services/webhookWorker.service.js";
...
// Detecta el error de clave duplicada de MongoDB (código 11000) sin usar any
...
export async function handleWebhookPost(req: Request, res: Response) {
  try {
    ...
    // Encola el mensaje para que el worker lo procese; se responde a Meta en cuanto queda guardado
    try {
      await WebhookEvent.create({ waId, messageId, payload: { msg, phoneNumberId: inboundPhoneNumberId } });
      // Fire-and-forget: no se espera el procesamiento para responder a Meta
      scheduleProcessing(waId);
    } catch (err: unknown) {
      // 11000: ese messageId ya estaba encolado, es un reintento de Meta → 200 sin crear otra fila
      if (isDuplicateKeyError(err)) return res.sendStatus(200);
      // Cualquier otro error (Mongo caído, validación...): 500 para que Meta reintente con su backoff
...
// Procesa un mensaje entrante ya parseado: ubicación, texto, media y máquina de estados.
// No conoce req/res; la respuesta HTTP la da handleWebhookPost.
export async function processIncomingMessage(params: {
  waId: string;
  messageId?: string | undefined;
  msg: any; // Excepción aprobada: mensaje crudo de WhatsApp sin tipar (antes era implícito)
  inboundPhoneNumberId?: string | undefined;
}): Promise<void> {
  const { waId, messageId, msg, inboundPhoneNumberId } = params;
